...
BfGraveyard::BfGraveyard(Battlefield* battlefield)
{
    ...
    for (uint8 i = 0; i < BG_TEAMS_COUNT; i++)
        m_SpiritGuide[i] = std::vector<ObjectGuid>();
}
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    ...
    m_SpiritGuide[team].push_back(spirit->GetGUID());
...
void BfGraveyard::Resurrect()
{
    ...
    for (GuidSet::const_iterator itr = m_ResurrectQueue.begin(); itr != m_ResurrectQueue.end(); ++itr)
    {
        ...
        if (player->IsInWorld() && m_SpiritGuide[m_ControlTeam].size())
        {
            if (Creature* spirit = m_Bf->GetCreature(m_SpiritGuide[m_ControlTeam][0]))
...
{
    if (!m_SpiritGuide[TEAM_ALLIANCE].size() || !m_SpiritGuide[TEAM_HORDE].size())
        return false;

    for (uint8 team = 0; team < TEAM_NEUTRAL; team++)
    {
        for (auto itr = m_SpiritGuide[team].begin(); itr != m_SpiritGuide[team].end(); itr++)
        {
            if (m_Bf->GetCreature(*itr) && (*itr) == guid)
                return true;
        }
    }
