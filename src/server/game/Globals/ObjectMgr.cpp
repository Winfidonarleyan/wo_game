...
void ObjectMgr::LoadForbiddenTexts()
{
    _forbiddenTexts.clear();

    uint32 oldMSTime = getMSTime();

    QueryResult result = LoginDatabase.Query("SELECT `text` FROM forbidden_texts");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 forbidden texts. DB table `forbidden_texts` is empty.");
        return;
    }

    uint32 count = 0;

    _forbiddenTexts.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        if (value.empty())
            TC_LOG_ERROR("sql.sql", "`forbidden_texts` contains an empty (or whitespace) value");
        else
            _forbiddenTexts.push_back(value);

        ++count;
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u forbidden texts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadEnchantVisual()
{
    uint32 oldMSTime = getMSTime();
    CharacterDatabase.Execute("DELETE FROM character_enchant_visual WHERE NOT EXISTS (SELECT 1 FROM item_instance WHERE item_instance.guid = character_enchant_visual.itemGuid)");
    
    QueryResult result = CharacterDatabase.Query("SELECT `itemGuid`, `visualEntry` FROM `character_enchant_visual`");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 visual enchants. DB table `character_enchant_visual` is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint32 enchantEntry = fields[1].GetUInt32();
        _enchantItemsStore[guid] = enchantEntry;
    }
    while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u visual enchants in %u ms", uint32(_enchantItemsStore.size()), GetMSTimeDiffToNow(oldMSTime));
}

uint32 ObjectMgr::GetEnchantVisual(ObjectGuid::LowType itemGuid)
{
    EnchantItemsContainer::const_iterator itr = _enchantItemsStore.find(itemGuid);
    if (itr != _enchantItemsStore.end())
        return itr->second;

    return 0;
}

void ObjectMgr::SetEnchantVisual(ObjectGuid::LowType itemGuid, uint32 visualEntry)
{
    _enchantItemsStore[itemGuid] = visualEntry;
}

void ObjectMgr::RemoveEnchantVisual(ObjectGuid::LowType itemGuid)
{
    EnchantItemsContainer::iterator itr = _enchantItemsStore.find(itemGuid);
    if (itr != _enchantItemsStore.end())
        _enchantItemsStore.erase(itr);
}