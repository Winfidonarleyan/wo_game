...
void BattlegroundSA::UpdateDemolisherHealth()
{
    static const uint32 minPlayers = 5, maxPlayers = 15;
    static const float minMod = 0.3333f, maxMod = 1.0f;

    auto defenderTeam = Attackers == TEAM_ALLIANCE ? HORDE : ALLIANCE;
    uint32 playerCount = GetPlayersCountByTeam(defenderTeam);
    
    if (!playerCount)
        return;

    float mod = (playerCount / (float)maxPlayers);
    ...
    for (uint8 i = BG_SA_DEMOLISHER_1; i <= BG_SA_DEMOLISHER_8; i++)
    {
        if (BgCreatures[i])
        {
            ...
            {
                if (!maxHealth)
                    maxHealth = Demolisher->GetMaxHealth();

                float currentPercent = Demolisher->GetHealth() == Demolisher->GetMaxHealth() ? -1 : 
                    Demolisher->GetHealth() / (float)Demolisher->GetMaxHealth();

                if (currentPercent > 1)
                    currentPercent = 1;

                Demolisher->SetMaxHealth(maxHealth * mod);

                if (!Demolisher->isDead() && currentPercent > 0)
                    Demolisher->SetHealth(Demolisher->GetMaxHealth() * currentPercent);
            }
        }
    }
}
