...
bool ArenaTeam::Create(ObjectGuid captainGuid, uint8 type, std::string const& teamName, uint32 backgroundColor, uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle, uint32 borderColor)
{
    ...
    Type = type;
    TeamName = (type == ARENA_TEAM_3v3) ? "[Solo Queue 3v3]: " + teamName : teamName;
...
void ArenaTeam::CreateTempForSolo3v3(Player* plr[], uint8 team)
{
    ...
    {
        ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById(plr[i]->GetArenaTeamId(GetSlotByType(ARENA_TEAM_3v3)));
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    if (GetType() == ARENA_TYPE_5v5 && GetMembersSize() == 1)
        return false;
    if (GetType() == ARENA_TYPE_3v3 && GetMembersSize() == 1)
        return false;
...
uint32 ArenaTeam::GetPoints(uint32 memberRating)
{
    ...
    if (Type == ARENA_TEAM_2v2)
        points *= 0.76f;
    else if (Type == ARENA_TEAM_3v3) // 3v3solo arena
