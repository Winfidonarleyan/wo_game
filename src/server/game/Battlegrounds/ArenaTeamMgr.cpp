...
void ArenaTeamMgr::DistributeArenaPoints()
{
    ...
    {
        float points = playerItr->second, bonusPoints = 0.0f;
        ...
        if (Player* player = ObjectAccessor::FindConnectedPlayer(ObjectGuid(HighGuid::Player, playerItr->first)))
        {
            ...
            if (points >= 1)
                player->ModifyArenaPoints((uint32)points, trans);
        }
        else    // Update database
        {
            sScriptMgr->OnGivePlayerArenaPoints(ObjectGuid(HighGuid::Player, playerItr->first), points, bonusPoints);
            ...
            if (points >= 1)
            {
                ...
                stmt->setUInt32(0, (uint32)points);
