...
void Battleground::CheckStartSolo3v3Arena()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        for (ArenaTeam::MemberList::iterator itr = team[i]->m_membersBegin(); itr != team[i]->m_membersEnd(); itr++)
        {
            ...
            if (!plr->IsSpectator() && plr->GetInstanceId() != GetInstanceID())
            {
    ...
    if (someoneNotInArena && sWorld->getBoolConfig(CONFIG_SOLO_3V3_STOP_GAME_INCOMPLETE) || plrinarena < 6)
    {
        SetRated(false);
        EndBattleground(0);
    }
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (team == winner)
        {
            if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetTypeID()))
            {
                ...
                {
                    float points = winner_arena, bonusPoints = 0.0f;

                    sScriptMgr->OnGivePlayerArenaPoints(player, points, bonusPoints);
...
void Battleground::RemovePlayerAtLeave(ObjectGuid guid, bool Transport, bool SendPacket)
{
    ...
    if (team == TEAM_SPECTATOR) // Spectator team
    {
        player->ResummonPetTemporaryUnSummonedIfAny();
    }
}
...
void Battleground::AddSpectator(Player* player)
{
    ...
    bp.Team = TEAM_SPECTATOR;

    m_Players[player->GetGUID()] = bp;
    PlayerAddedToBGCheckIfBGIsRunning(player);
}

bool Battleground::IsSpectator(const Player* player) const
{
    return GetPlayerTeam(player->GetGUID()) == 35;
}
...
void Battleground::EventPlayerLoggedOut(Player* player)
{
    ...
    {
        RemovePlayerAtLeave(guid, true, false);
        return;
    }
...
uint32 Battleground::GetAlivePlayersCountByTeam(uint32 Team) const
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        {
            Player* player = ObjectAccessor::FindPlayer(itr->first);
            if (player && player->IsAlive() && player->GetShapeshiftForm() != FORM_SPIRITOFREDEMPTION && !player->IsSpectator())
                ++count;
        }
    }
    return count;
