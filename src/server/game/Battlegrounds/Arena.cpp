            ...
            for (auto const& i : GetPlayers())
            {
                ...
                if (team == winner)
                {
                    if (!AntiWintrade)
                    {
                        ...
                        if (winnerArenaTeam->GetSlot() == ARENA_TYPE_3v3)
                            rating = player->GetArenaPersonalRating(ARENA_TEAM_3v3); // Try find 3v3 SoloQueue arena team ARENA_TEAM_3v3 == 3
                        if (rating == 0) // if can't find 3v3 SoloQueue, try find Type 5 arena team
