...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    {
        if (auto rbTemplate = GetBattlegroundTemplateByTypeId(originalBgTypeId))
        {
            bg->SetMinPlayersPerTeam(rbTemplate->MinPlayersPerTeam);
            bg->SetMinPlayers(rbTemplate->MinPlayersPerTeam * 2);
        }
    }
...
std::vector<Battleground*> BattlegroundMgr::GetBattlegrounds() const
{
    std::vector<Battleground*> list;

    for (auto itr1 = bgDataStore.begin(); itr1 != bgDataStore.end(); itr1++)
    {
        auto &bgs = itr1->second.m_Battlegrounds;

        for (auto itr2 = bgs.begin(); itr2 != bgs.end(); itr2++)
        {
            if (itr2->second->GetStatus() != STATUS_NONE)
                list.push_back(itr2->second);
        }
    }
