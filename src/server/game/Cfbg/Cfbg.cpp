...
void Player::SetFakeRaceAndMorph()
{
    if (getClass() == CLASS_DRUID)
    {
        ...
        else
        {
            ...
            m_FakeMorph = GetCFSTeam() == ALLIANCE ? 20579 : 19724;
        }
    }
...
bool Player::SendBattleGroundChat(uint32 msgtype, Language lang, std::string message)
{
    ...
    if (Battleground* pBattleGround = GetBattleground())
    {
        ...
        for (Battleground::BattlegroundPlayerMap::const_iterator itr = pBattleGround->GetPlayers().begin(); itr != pBattleGround->GetPlayers().end(); ++itr)
        {
            ...
            {
                if (GetDistance2d(target->GetPositionX(), target->GetPositionY()) <= distance)
                {
                    WorldPacket data(SMSG_MESSAGECHAT, 200);
                    Language targetLang = GetTeam() == target->GetTeam() || target == this ?
                        (target->GetCFSTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH) :
                        (target->GetCFSTeam() == ALLIANCE ? LANG_ORCISH : LANG_COMMON);

                    BuildPlayerChat(&data, msgtype, message, targetLang);
