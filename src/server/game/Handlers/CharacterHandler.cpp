...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    {
        if (res != CHAR_NAME_MIXED_LANGUAGES || !IsFake()) // Skip mixed check for fake
        {
            SendCharCreate(res);
            return;
        }
        ...
        .WithChainingPreparedCallback([this, createInfo](QueryCallback& queryCallback, PreparedQueryResult result)
    {
        ...
        std::function<void(PreparedQueryResult)> finalizeCharacterCreation = [this, createInfo](PreparedQueryResult result)
        {
            ...
            bool allowTwoSideAccounts = !sWorld->IsPvPRealm() || HasPermission(rbac::RBAC_PERM_TWO_SIDE_CHARACTER_CREATION);
            uint32 skipCinematics = sWorld->getIntConfig(CONFIG_SKIP_CINEMATICS);
            bool checkDeathKnightReqs = !IsFake() && createInfo->Class == CLASS_DEATH_KNIGHT && !HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHARACTER_CREATION_DEATH_KNIGHT);
            ...
            if (IsFake())
                newChar.SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK20 | PLAYER_FLAGS_UNK21);
            ...
            LastCreatedCharGuid = newChar.GetGUID();
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    if (ArenaTeam* arena = sArenaTeamMgr->GetArenaTeamByCaptain(guid))
    {
        ...
        if (arena->GetType() < ARENA_TYPE_3v3 || arena->GetType() < ARENA_TYPE_5v5)
        {
            sScriptMgr->OnPlayerFailedDelete(guid, initAccountId);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    if (!IsFake() && !IsLegitCharacterForAccount(playerGuid))
    {
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    if (!pCurrChar->getCinematic())
    {
        ...
        if (ChrClassesEntry const* cEntry = sChrClassesStore.LookupEntry(pCurrChar->getClass()))
        {
            ...
            else if (ChrRacesEntry const* rEntry = sChrRacesStore.LookupEntry(pCurrChar->getCFSRace()))
    ...
    if (firstLogin)
    {
        ...
        PlayerInfo const* info = sObjectMgr->GetPlayerInfo(pCurrChar->getCFSRace(), pCurrChar->getClass());
...
void WorldSession::HandleAlterAppearance(WorldPacket& recvData)
{
    ...
    if (!bs_hair || bs_hair->type != 0 || bs_hair->race != _player->getCFSRace() || bs_hair->gender != _player->GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_GENDER))
        return;

    BarberShopStyleEntry const* bs_facialHair = sBarberShopStyleStore.LookupEntry(FacialHair);

    if (!bs_facialHair || bs_facialHair->type != 2 || bs_facialHair->race != _player->getCFSRace() || bs_facialHair->gender != _player->GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_GENDER))
        return;

    BarberShopStyleEntry const* bs_skinColor = sBarberShopStyleStore.LookupEntry(SkinColor);

    if (bs_skinColor && (bs_skinColor->type != 3 || bs_skinColor->race != _player->getCFSRace() || bs_skinColor->gender != _player->GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_GENDER)))
        return;

    if (!Player::ValidateAppearance(_player->getCFSRace(), _player->getClass(), _player->GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_GENDER),
        bs_hair->hair_id,
        Color,
