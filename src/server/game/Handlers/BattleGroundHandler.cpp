...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    /*
    if (type == ARENA_TYPE_3v3 && _player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_3v3))
        type = ARENA_TYPE_3v3; 
 
    if (type == ARENA_TYPE_5v5 && _player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
        type = ARENA_TYPE_5v5;
     */
    ...
    if (action)
    {
        ...
        if(bgQueueTypeId == BATTLEGROUND_QUEUE_5v5 && Arena3v3CheckTalents(_player) == false)
            return;
