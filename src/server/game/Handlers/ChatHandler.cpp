...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    else
    {
        ...
        {
            auto t = time(nullptr);
            long diff = m_muteTime - t;
            if (diff < 0)
                diff = (60 * 15) - sender->GetTotalPlayedTime();
			
            std::string timeStr = secsToTimeString(diff);
    ...
    if (!GetPlayer()->IsGameMaster() && GetPlayer()->SendBattleGroundChat(type, Language(lang), msg))
        return;
