...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    if (!m_Socket)
    {
        if (IsFake())
        {
            for (auto itr = _catchHandlers.begin(); itr != _catchHandlers.end();)
            {
                if (itr->first == packet->GetOpcode() && itr->second(*((WorldPacket*)packet)))
                    itr = _catchHandlers.erase(itr);
                else
                    itr++;
            }
        }
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
    ...
    while ((m_Socket || IsFake()) && _recvQueue.next(packet, updater))
    {
        ClientOpcodeHandler const* opHandle = opcodeTable[static_cast<OpcodeClient>(packet->GetOpcode())];
        try
        {
...
#define MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE 100
    ...
    if (updater.ProcessLogout() && !IsFake())
    {
...
void WorldSession::SetFake(bool fake)
{
    _isFake = fake;
}
...
void WorldSession::CatchPacket(Opcodes opcode, CatchHandler func)
{
    if (!m_Socket && IsFake())
    {
        _catchHandlers.push_back(std::pair<Opcodes, CatchHandler>(opcode, func));
    }
}
