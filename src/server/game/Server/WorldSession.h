...
class TC_GAME_API WorldSession
{
    ...
    public:
        ...
        typedef std::function<bool(WorldPacket&)> CatchHandler;
        void CatchPacket(Opcodes opcode, CatchHandler func);

        ObjectGuid LastCreatedCharGuid;
    ...
    private:
     ...
     // std::string m_LAddress;                             // Last Attempted Remote Adress - we can not set attempted ip for a non-existing session!
        ...
        bool _isFake;
        std::list<std::pair<Opcodes, CatchHandler>> _catchHandlers;
};
#endif
/// @}
