...
const char* Transmogrification::GetSlotName(uint8 slot, WorldSession* session) const
{
    ...
    switch (slot)
    {
        ...
        case EQUIPMENT_SLOT_BODY: return session->GetTrinityString(LANG_SLOT_NAME_SHIRT);
...
bool Transmogrification::CanTransmogrifyItemWithItem(Player* player, ItemTemplate const* target, ItemTemplate const* source, uint8 slot) const
{
    ...
    if (source->InventoryType != target->InventoryType)
    {
        ...
        {
            if (slot == SLOT_MAIN_HAND && source->InventoryType == INVTYPE_WEAPONMAINHAND)
                return true;

            if (slot == SLOT_OFF_HAND && source->InventoryType == INVTYPE_WEAPONOFFHAND)
                return true;
        }
...
bool Transmogrification::SuitableForTransmogrification(Player* player, ItemTemplate const* proto) const
{
    ...
    if (!IgnoreReqRace && (proto->AllowableRace & player->getCFSRaceMask()) == 0)
        return false;
