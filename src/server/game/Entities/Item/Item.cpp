...
bool Item::LoadFromDB(ObjectGuid::LowType guid, ObjectGuid owner_guid, Field* fields, uint32 entry)
{
    ...
    if (uint32 visualEntry = sObjectMgr->GetEnchantVisual(guid))
        SetEnchantVisual(visualEntry);
...
void Item::SetEnchantVisual(uint32 visualEntry)
{
    if (m_enchantVisualEntry != visualEntry)
    {
        sObjectMgr->SetEnchantVisual(GetGUID().GetCounter(), visualEntry);

        (!m_enchantVisualEntry) ? CharacterDatabase.PExecute("REPLACE INTO character_enchant_visual VALUES (%u, %u)", GetGUID().GetCounter(), visualEntry) :
                                CharacterDatabase.PExecute("UPDATE character_enchant_visual SET visualEntry = '%u' WHERE itemGuid = '%u'", visualEntry, GetGUID().GetCounter());

        m_enchantVisualEntry = visualEntry;
    }
}

void Item::RemoveEnchantVisual()
{
    if (GetEnchantVisual())
    {
        m_enchantVisualEntry = 0;
        sObjectMgr->RemoveEnchantVisual(GetGUID().GetCounter());
        CharacterDatabase.PExecute("DELETE FROM character_enchant_visual WHERE itemGuid = %u", GetGUID().GetCounter());
    }
}