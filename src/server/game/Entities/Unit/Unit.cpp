...
Unit::Unit(bool isWorldObject) :
    ...
    m_HostileRefManager(this), m_comboTarget(nullptr), m_comboPoints(0), m_spellHistory(new SpellHistory(this)), _speedhackCooldown(0)
{
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (cleanDamage && damagetype == DIRECT_DAMAGE && this != victim && getPowerType() == POWER_RAGE)
    {
        ...
        uint32 rage_damage = d_damage + cleanDamage->absorbed_damage;
        ...
        switch (cleanDamage->attackType)
        {
            ...
            case OFF_ATTACK:
            {
                weaponSpeedHitFactor = uint32(GetAttackTime(cleanDamage->attackType) / 1000.0f * (this->ToPlayer()->CanTitanGrip() ? 3.5f : 1.75f));
                if (cleanDamage->hitOutCome == MELEE_HIT_CRIT)
                    weaponSpeedHitFactor *= 2;
    ...
    if (!GetMap()->IsBattleArena()|| !GetAreaId() == 616)
    {
        if (ToCreature() && ToCreature()->GetInstanceId())
            damage *= 0.01f;
    }
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    if (aurApp->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE && GetTypeId() == TYPEID_UNIT && IsTotem())
    {
        if (ToTotem()->GetSpell() == aura->GetId() && ToTotem()->GetTotemType() == TOTEM_PASSIVE)
        {
            if (IsMagnet())
            {
                //if UnSummon parameter(ms) is too low it won't show Death Grip animation effect
                ToTotem()->UnSummon(175);
            }
            else
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype) const
{
    ...
    {
        //AP: TEMP HACK for easymode
        switch(ToCreature()->GetEntry())
        {
            case 36839: // Gunship Cannon
            case 36838:
                DoneTotalMod *= 0.3f;
                break;
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    uint32 damageVal = pdamage;
    uint32 damageIgnoreMods = 0;

    if (damageVal > 1)
    {
        if (auto casterPlayer = caster->ToPlayer())
        {
            if (casterPlayer->HasAura(31884)) // Paladin wings
            {
                if (casterPlayer->HasAura(53375)) // rank 1 divine wrath
                {
                    damageVal = pdamage * 0.75f;
                    damageIgnoreMods = pdamage * 0.25f;
                }
                else if (casterPlayer->HasAura(53376)) // rank 2 divine wrath
                {
                    damageVal = damageIgnoreMods = pdamage * 0.5f;
                }
            }
        }
    }
    ...
    if (TakenTotalCasterMod)
    {
        ...
        {
            if (TakenTotalMod < 1)
                tmpDamage = ((float(CalculatePct(damageVal, TakenTotalCasterMod) + TakenTotal) * TakenTotalMod) + CalculatePct(damageVal, TakenTotalCasterMod));
            else
                tmpDamage = ((float(CalculatePct(damageVal, TakenTotalCasterMod) + TakenTotal) + CalculatePct(damageVal, TakenTotalCasterMod)) * TakenTotalMod);
        }
        else if (TakenTotalMod < 1)
            tmpDamage = ((CalculatePct(float(damageVal) + TakenTotal, TakenTotalCasterMod) * TakenTotalMod) + CalculatePct(float(damageVal) + TakenTotal, TakenTotalCasterMod));
    }

    if (!tmpDamage)
        tmpDamage = (float(damageVal) + TakenTotal) * TakenTotalMod;

    tmpDamage += damageIgnoreMods;
...
void Unit::RewardRage(uint32 damage, uint32 weaponSpeedHitFactor, bool attacker)
{
    ...
    if (attacker)
    {
        addRage = ((damage / rageconversion) * 3.75f) + (weaponSpeedHitFactor / 2.0f);
...
bool Unit::CheckMovementInfo(MovementInfo const& movementInfo)
{
    ...
    float timer = GetLastMoveClientTimestamp();
    if (timer)
    {
        ...
        {
            if (GetPlayerMovingMe()->IsGameMaster())
                return true;
        ...
        float difftime = (movetime - timer) * 0.001f + delay;
