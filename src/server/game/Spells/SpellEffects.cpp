        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x200 && m_spellInfo->GetCategory() == 1209)
                {
                    ...
                    if (m_caster->HasAura(58374))
                        block_value_multiplier += .1f;
                    if (m_caster->HasAura(2565))
                        h_cap = h_cap * 2;

                    uint32 cap = static_cast<uint32>(h_cap * block_value_multiplier);
                    block *= block_value_multiplier;
                    uint32 bonusdamage = static_cast<uint32>(block > cap ? cap + (block - cap) * 0.0933f : block);
                    uint32 full_dmg = damage + bonusdamage;

                    if(AuraEffect* AuraEff= m_caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_WARRIOR, 280, 1))
                        full_dmg *= AuraEff->GetAmount() / 100.0f + 1.0f;
...
void Spell::EffectLearnSkill(SpellEffIndex effIndex)
{
    ...
    SkillRaceClassInfoEntry const* rcEntry = GetSkillRaceClassInfo(skillid, unitTarget->getCFSRace(), unitTarget->getClass());
    if (!rcEntry)
        return;
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                // Toss Your Luck! (item Titanium Seal of Dalaran (44430))
                case 60476:
                {
                    Player* player = m_caster->ToPlayer();
                    ...
                    player->TextEmote(player->GetSession()->GetTrinityString(gender ? LANG_ITEM_TITANIUM_SEAL_OF_DALARAN_FEMALE1 : LANG_ITEM_TITANIUM_SEAL_OF_DALARAN_MALE1), 0);
                    player->TextEmote(player->GetSession()->GetTrinityString(urand(0, 1) ? LANG_ITEM_TITANIUM_SEAL_OF_DALARAN_UP : LANG_ITEM_TITANIUM_SEAL_OF_DALARAN_DOWN), 0);
                    break;
                }
            }
            break;
