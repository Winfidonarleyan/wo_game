...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (GetCaster()->GetTypeId() == TYPEID_UNIT && _triggeredCastFlags == 0)
    {
        if (GetCaster()->HasAura(49560))
        {
    ...
    switch (m_spellState)
    {
        case SPELL_STATE_PREPARING:
        {
            ...
            if (m_timer == 0 && !m_spellInfo->IsNextMeleeSwingSpell() && !IsAutoRepeat())
                ...
                cast(!m_casttime && !m_castItemGUID); // Repeat check for item casts (bug with copying items)
            break;
        }
        case SPELL_STATE_CASTING:
        {
...
SpellCastResult Spell::CheckCast(bool strict, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_caster->GetEntry() != WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly cast by a trigger)
            {
                ...
                if (!IsTriggered() && !m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, nullptr, SPELL_DISABLE_LOS) && !target->IsWithinLOSInMap(losTarget, LINEOFSIGHT_ALL_CHECKS))
    ...
    if (m_targets.HasDst())
    {
        ...
        if (!IsTriggered() && !m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, nullptr, SPELL_DISABLE_LOS) && !m_caster->IsWithinLOS(x, y, z, LINEOFSIGHT_ALL_CHECKS))
            return SPELL_FAILED_LINE_OF_SIGHT;
    }
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_TALENT_SPEC_SELECT:
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if (plr->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_3v3) ||
                        plr->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
                        {
                            plr->GetSession()->SendNotification(LANG_SOLO_ARENA_CHANGE_TALENTS);
                            return SPELL_FAILED_DONT_REPORT;
                        }
