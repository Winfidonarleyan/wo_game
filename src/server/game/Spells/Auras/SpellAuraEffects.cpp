...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        {
            float pct = GetDonePct();

            if (m_spellInfo->IsRankOf(sSpellMgr->GetSpellInfo(1120))) // Warlock drain soul
                pct = caster->SpellDamagePctDone(target, m_spellInfo, DOT);

            damage = std::max(int32(damage * pct), 0);
        }
