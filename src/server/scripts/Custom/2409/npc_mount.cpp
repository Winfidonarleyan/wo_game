...
public:
    ...
    struct npc_mountAI : public ScriptedAI
    {
        npc_mountAI(Creature* creature) : ScriptedAI(creature) { }

        bool OnGossipHello(Player* pPlayer, Creature* pCreature)
        {
            AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, MSG_GOSSIP_TEXT_MOUNT, GOSSIP_SENDER_MAIN, 1);
            SendGossipMenuFor(pPlayer, DEFAULT_GOSSIP_MESSAGE, pCreature);
            return true;
        }
        ...
        bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
        {
            ...
            case 1:
                ...
                {
                    pCreature->Whisper(MSG_HI_LEVEL, LANG_UNIVERSAL, pPlayer);
                    CloseGossipMenuFor(pPlayer);
                    break;
                }
                if (pPlayer->IsMounted())
                {
                    pCreature->Whisper(MSG_MOUTED, LANG_UNIVERSAL, pPlayer);
                    CloseGossipMenuFor(pPlayer);
                    break;
                }
    ...
    {
        return new npc_mountAI(creature);
    }
};
