        ...
        void EventEnd()
        {
            ...
            if (Group* EventGroup = player->GetGroup())
            {
                ...
                for (Group::member_citerator itr = members.begin(); itr != members.end(); ++itr)
                {
                    ...
                    if (groupMember->GetQuestStatus(QUEST_A_PAWN_ON_THE_ETERNAL_BOARD) == QUEST_STATUS_INCOMPLETE)
                        groupMember->SetQuestStatus(QUEST_A_PAWN_ON_THE_ETERNAL_BOARD, QUEST_STATUS_COMPLETE, true);
                }
            }
        }
