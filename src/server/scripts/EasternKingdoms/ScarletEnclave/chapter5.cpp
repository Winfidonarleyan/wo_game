    ...
    struct npc_highlord_darion_mograineAI : public EscortAI
    {
        ...
        void Reset() override
        {
            if (!HasEscortState(STATE_ESCORT_ESCORTING))
            {
                ...
                uiStep = 0;
                uiPhase_timer = 3000;
                uiFight_duration = 60000; // 5 minutes (1 minute)
...
class dk_playerScript : PlayerScript
{
public:
    dk_playerScript() : PlayerScript("dk_last_quest_script") { }
    ...
    void OnQuestStatusChange(Player* player, uint32 questId) override
    {
        if (questId == 13188)
        {
            if (player->GetQuestStatus(questId) == QUEST_STATUS_REWARDED)
                player->RemoveAurasDueToSpell(58530);
        }
        else if (questId == 13189)
        {
            if (player->GetQuestStatus(questId) == QUEST_STATUS_REWARDED)
                player->RemoveAurasDueToSpell(58551);
        }
    }
};
