...
public:
    ...
    class spell_dk_dancing_rune_weapon_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            static std::vector<uint32> allowedSpells
            {
            ...
            {
                for (auto itr = allowedSpells.begin(); itr != allowedSpells.end(); itr++)
                {
                    if (procSpell->IsRankOf(sSpellMgr->GetSpellInfo(*itr)))
                        return true;
                }
            }
        ...
        void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            runeWeapon->CastSpell(runeWeapon->GetVictim(), procSpell, TriggerCastFlags(TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD | TRIGGERED_IGNORE_POWER_AND_REAGENT_COST));
        }
...
public:
    bool operator()(WorldObject* target) const
    {
        if (Unit* unit = target->ToUnit())
            return unit->HasAuraType(SPELL_AURA_MOD_STEALTH);
        return false;
    }
};
    ...
    public:
        ...
        class spell_dk_pestilence_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targetList)
            {
                ...
                if (victim->GetAura(SPELL_DK_BLOOD_PLAGUE, caster->GetGUID()) || victim->GetAura(SPELL_DK_FROST_FEVER, caster->GetGUID()))
                    targetList.remove_if(StealthCheck());
                else
                    targetList.clear();
            }
...
public:
    ...
    class spell_dk_death_grip_initial_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckCast()
        {
            ...
            // Patch 3.3.3 (2010-03-23): Minimum range has been changed to 8 yards in PvP.-This not correct this is test world.Not live.
