            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_START_EVENT:
                    {
                        ...
                        GetCreatureListWithEntryInGrid(creatures, me, NPC_SE_KOR_KRON_REAVER, 20.0f);
                        creatures.sort(Trinity::ObjectDistanceOrderPred(me));

                        if (creatures.size() > 5)
                        {
                            uint32 x = 1;
                            for (std::list<Creature*>::iterator itr = creatures.begin(); itr != creatures.end(); ++x)
                            {
                                if (x > 5)
                                {
                        ...
                        for (Creature* creature : creatures)
                            _guardList.push_back(creature->GetGUID());

                        uint32 x = 1;
                        for (GuidList::iterator itr = _guardList.begin(); itr != _guardList.end(); ++x, ++itr)
                            if (Creature* creature = ObjectAccessor::GetCreature(*me, *itr))
                                creature->AI()->SetData(0, x);
                    ...
                    case ACTION_START_OUTRO:
                    {
                        ...
                        for (GuidList::const_iterator itr = _guardList.begin(); itr != _guardList.end(); ++itr)
                            if (Creature* creature = ObjectAccessor::GetCreature(*me, *itr))
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_START_EVENT:
                    {
                        ...
                        std::list<Creature*> creatures;
                        GetCreatureListWithEntryInGrid(creatures, me, NPC_SE_SKYBREAKER_MARINE, 20.0f);
                        creatures.sort(Trinity::ObjectDistanceOrderPred(me));
                        ...
                        uint32 x = 1;
                        for (GuidList::const_iterator itr = _guardList.begin(); itr != _guardList.end(); ++x, ++itr)
                            if (Creature* creature = ObjectAccessor::GetCreature(*me, *itr))
                                creature->AI()->SetData(0, x);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_ALLIANCE_5:
                            ...
                            for (std::list<ObjectGuid>::iterator itr = _guardList.begin(); itr != _guardList.end(); ++itr)
                                if (Creature* creature = ObjectAccessor::GetCreature(*me, *itr))
                                    creature->AI()->DoAction(ACTION_CHARGE);
                            me->GetMotionMaster()->MoveCharge(chargePos[0].GetPositionX(), chargePos[0].GetPositionY(), chargePos[0].GetPositionZ(), 8.5f, POINT_CHARGE);
                            break;
