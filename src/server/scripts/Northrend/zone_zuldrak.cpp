...
class spell_harvest_blight_crystal : public SpellScriptLoader
{
public: spell_harvest_blight_crystal() : SpellScriptLoader("spell_harvest_blight_crysta") { }

        class spell_harvest_blight_crystal_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_harvest_blight_crystal_SpellScript);

            SpellCastResult CheckCast()
            {
                if (GetCaster()->FindNearestGameObject(190940, 5.0f))
            ...
            {
                if(GetCaster()->GetCharmer())
                    GetCaster()->GetCharmer()->ToPlayer()->KilledMonsterCredit(28740);
            }

            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_harvest_blight_crystal_SpellScript::CheckCast);
                OnEffectHitTarget += SpellEffectFn(spell_harvest_blight_crystal_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_harvest_blight_crystal_SpellScript();
        }
};
...
void AddSC_zuldrak()
{
    ...
    new npc_storm_cloud();
    new spell_harvest_blight_crystal();
}
